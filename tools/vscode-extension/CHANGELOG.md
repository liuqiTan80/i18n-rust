...
### 新增
- **AI 设置防外传**：provider / baseUrl / apiKey 限定用户级（`application` 作用域，工作区不可覆盖）——不可信仓库曾可把 baseUrl 指向第三方使明文密钥外发；激活时对历史工作区覆盖给出明确警告。

### 修复
- **Windows 命令注入残留根治**：终端命令转义按实际 shell 分派（PowerShell 单引号字面量 / cmd `^` 逐字符 / Git Bash 归 POSIX）——此前按 `os.platform` 二分，PowerShell 下 `^%` 反而失效。
- **插件薄弱点加固**：终端按 cwd 复用（跨目录不再各自新建）、密钥迁移逐层容错（单层失败不阻塞）、LSP 自动重启改 10 分钟时间窗口（偶发崩溃不再跨周月累计）、语言包探测抽共享模块（删除硬编码开发机路径）。

## 0.8.1

### 新增
- **zh 词表扩充**：新增密码学表（sha2 / md5 / hmac / pbkdf2 / aes / cbc / zeroize / ed25519_dalek / getrandom / hex 共 10 个 crate 的 API 映射）；工具 / 数据库 / 日志 / 命令行四表补条（weix-1 全量中文化实战定稿）。

### 修复
- **IDE 诊断误报根治**：打开 / 保存即执行与 `rzc` 构建同口径的权威检查——真实项目镜像 cargo check（离线、复用依赖编译产物），编译级假红（E 系列 error 与 companion hint）及「未打开模块 / 依赖缺失 / 资源缺失」类误报清零；模块名与类型名重名导致的转译前缀误加（E0425 假红）一并根治。教学提示（未标注 let、魔法数字、易混方法名）与「rzc add」添加依赖建议完整保留。

## 0.8.0

### 新增
- **转译产物显示开关**：新增设置 `i18n-rust.hideGeneratedFiles` 与命令「i18n: 显示/隐藏转译产物 (.rs / .rs.bak)」——一键把转译生成的 `.rs` 与 `.rs.bak` 从资源管理器隐藏或恢复显示（写入工作区 `files.exclude`）；开关状态按工作区保存，激活与设置变化时自动同步；可在设置面板或命令面板切换。
- **教学 lint 新增「易混方法名」规则**：方法调用位出现未收录的长中文串且疑似拼写错误时，IDE 提示相近的正确写法（如 `.拉平()` → 建议「展平」）。

### 修复
- **诊断翻译修复**：E0599「找不到方法」（no method named …）类诊断不再把消息中的 "found" 误译为「实际为」；「实际为」现仅用于类型不匹配（expected …, found …）场景。

## 0.7.3

### 修复
- **诊断误报修复**：中文模块声明（如 `模块 日志设置;`）不再触发 E0583（找不到模块文件）与 E0754（非 ASCII 标识符）满屏红色波浪线；多文件项目的 unlinked-file（“未包含在模块树中”）误报修复；引用未打开模块文件的 E0433 误报过滤（拼写错误仍正常提示）。
- **诊断翻译补全**：模块相关诊断消息（找不到模块文件、非 ASCII 标识符、未包含在模块树等）全部汉化，并修复短语替换顺序导致的半中半英。

## 0.7.2

### 新增
- **离线更新提醒**：扩展每天自动检查 GitHub Releases 上的新版本（可用 `i18n-rust.checkUpdates` 关闭），发现新版本时弹窗提示并提供「查看更新」入口——离线安装（vsix）的用户也能及时获知版本更新。

## 0.7.1

- 与 rzc 0.7.1 同步发布：GitCode 默认源随仓库改名更新（tan80/i18n-rust）。
...
### 新增
- **教程代码全中文化**：特征方法/关联类型/属性名/属性参数/宏片段说明符/布局指定符全部支持中文写法（`格式化方法`→fmt、`货物类型`→Item、`#[应恐慌(预期 = ...)]`→`#[should_panic(expected = ...)]`、`$名字:表达式`→`$名字:expr`、`#[布局(西)]`→`#[repr(C)]`），转译器自动转换为英文原名。
- **中文生命周期名**：支持 `'生命` 等中文生命周期标注。

### 修复
- **语言包映射补全**：新增标准库成员节（next/from/fmt/add/Item/Output/repr/should_panic/expr/ident/literal/ty）、消息传递（mpsc）、执行器（executor）等映射；`引用计数` 模块路径映射改名为 `引用计数模块`，避免 use 语句类型段被误映射为 rc 模块。
- **异步语言包重复键修复**：删除 异步.toml 中重复的 `执行器` 条目（曾导致语言包 TOML 解析失败）。

### 文档
- 33 章教程代码块全面中文化（生命周期、错误处理、特征、智能指针、并发、异步、FFI 等章节）；附录A 新增标准库成员映射表节。
