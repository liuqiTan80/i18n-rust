...
- **모국어 프로그래밍**: 한국어 키워드(`함수`, `선언`, `만약`, `반환`…)로 완전한 Rust 프로그램 작성
- **다국어 네이티브**: 모든 자연어를 네이티브로 지원; 11개 언어 팩 내장, 다른 언어는 원격 설치 가능
- **확장자 자동 감지**: `.zh`, `.en`, `.de` 등이 해당 언어 팩에 자동 매칭
...
**Q: 한국어 변수명을 사용할 수 있나요?**
네. `숫자`와 `메인`은 유효한 식별자입니다. Rust는 Unicode 식별자를 지원합니다.

**Q: 다른 언어 팩은 어떻게 설치하나요?**
`rzc lang install ja` (원격) 또는 `rzc lang install ./디렉토리` (로컬).
