...
- **扩展名自动识别**：`.zh`、`.ja`、`.ru` 等扩展名自动匹配对应语言包，无需手动指定
- **中文错误诊断**：解析 rustc 输出翻译为中文，附带 💡 教学提示；所有权错误（E0382/E0502/E0507）输出叙事化提示，并附结构化 JSON 供可视化
...
rzc 内置中文语言包（115+ 关键字、496 标准库映射、53 个错误码翻译），开箱即用。

- **扩展名自动识别**：`main.zh` 自动使用中文包；安装日语包后，`main.ja` 自动加载日语映射
- **远程安装**：`rzc lang install 俄语` 默认从 GitCode 下载，失败自动回退 GitHub
...
支持中文 Rust 语法高亮、智能补全、错误提示、一键运行/检查/导出与所有权可视化。在 VS Code 扩展市场搜索 `i18n-rust` 安装。
...
**Q：编译错误提示是英文怎么办？**
使用 `rzc check`，它会将 rustc 错误翻译为中文并附带教学提示。

**Q：如何安装其他语言包？**
`rzc lang install 日语`（远程）或 `rzc lang install ./语言包目录`（本地）。
...
欢迎通过 [GitHub Issues](https://github.com/liuqiTan80/i18n-rust/issues) 提交反馈、通过 Pull Request 贡献代码。
